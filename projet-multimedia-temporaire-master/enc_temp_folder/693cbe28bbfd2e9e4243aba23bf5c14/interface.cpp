...
static void InterfaceMouseCallback(int event, int x, int y, int flags, void* userdataAndMore) {
	...
	if (group->resetButton->isClicked(x, y)) {
		std::cout << "Bouton Réinitialiser l'image cliqué !" << std::endl;
		group->pCurrentImage->release(); 
		*(group->pCurrentImage) = cv::imread(filename); 
		if (group->pCurrentImage->empty()) {
			std::cerr << "Erreur lors du rechargement de l'image." << std::endl;
		}
	}
...
void Interface(string filename) {
	...
	Button resetButton(spacing * 8, 120, 150, btnHeight, "Réinitialiser l'image");
